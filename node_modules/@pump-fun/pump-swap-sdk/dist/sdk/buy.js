...
function buyBaseInputInternal({
  ...
  const coinCreatorFee = import_web32.PublicKey.default.equals(coinCreator) ? new import_bn3.default(0) : fee(quoteAmountIn, coinCreatorFeeBps);
...
function buyQuoteInputInternal({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(import_web32.PublicKey.default.equals(coinCreator) ? new import_bn3.default(0) : coinCreatorFeeBps);
  const denominator = new import_bn3.default(1e4).add(totalFeeBps);
  const effectiveQuote = quote.mul(new import_bn3.default(1e4)).div(denominator);
